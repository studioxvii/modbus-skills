...
runtime module, and helper script. `scripts/build_plugin_variants.py` derives four
...
- Cursor uses `.cursor-plugin/plugin.json`, native skill discovery, and no Codex agent metadata.
...
The Claude field is a packaging transform, not canonical content. It keeps Claude
skills manually invocable as `/modbus-skills:<skill-name>` while preventing automatic
model invocation. The portable package does not invent a cross-client invocation
policy where the standard has none. Variant validation compares the complete file
sets and bytes, removes the one documented Claude adapter before comparing skill
