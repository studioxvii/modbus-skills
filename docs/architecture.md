...
Human review is an explicit gate. It is not a skill that silently promotes evidence. The human produces a `modbus-review-decisions/v1` record. `apply-modbus-review-decisions` applies only its whitelisted, evidence-backed changes to a new map. A final tool adapter consumes only a reviewed map and a compiled read plan.
